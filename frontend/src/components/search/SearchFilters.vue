...
// Bộ lọc cục bộ - loại bỏ category vì không dùng filter nữa
const localFilters = reactive({
  documentType: props.currentFilters.documentType || "",
  author: props.currentFilters.author || "",
});

// Computed - loại bỏ category khỏi hasActiveFilters
...
const fetchAuthors = async () => {
  ...
  try {
    const response = await authorService.getAuthorsByBookKeyword(props.keyword);
    if (response.success) {
...
// Theo dõi thay đổi - loại bỏ category
watch(
...
watch(
  () => props.keyword,
  () => {
    localFilters.author = ""; // Reset bộ lọc tác giả khi từ khóa thay đổi
    fetchAuthors();
  }
);
